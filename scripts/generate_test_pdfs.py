...
def create_iso_27001_extract(filename):
    ...
    c.drawString(1 * inch, height - 1 * inch, "ISO/IEC 27001:2013 (Extract for Testing)")
    ...
    Control: An access control policy shall be established, documented and reviewed based on 
    business and information security requirements.
    
    A.9.1.2 Access to networks and network services
    Control: Users shall only be provided with access to the network and network services 
    ...
    Control: A formal user registration and de-registration process shall be implemented to 
    enable assignment of access rights.
    
    A.9.2.2 User access provisioning
    Control: A formal user access provisioning process shall be implemented to assign or 
    ...
    Control: Access to information and application system functions shall be restricted in 
    ...
    Control: A policy on the use of cryptographic controls for protection of information shall 
    be developed and implemented.
    
    A.10.1.2 Key management
    Control: A policy on the use, protection and lifetime of cryptographic keys shall be developed 
...
def create_fake_mobile_app_project(filename):
    ...
    # Title
    c.setFont("Helvetica-Bold", 18)
    c.drawString(1 * inch, height - 1 * inch, "Project: SecurePay Mobile App - Architecture Overview")
    
    # Metadata
    c.setFont("Helvetica", 10)
    c.drawString(1 * inch, height - 1.3 * inch, "Version: 1.0 | Date: 2026-03-06 | Confidential")
    ...
    text_obj.textLines("""
    SecurePay is a new mobile application designed to allow users to send money to friends 
    instantly using their phone number. The app will be available on iOS and Android.
    ...
    text_obj.textLines("""
    3.1 User Login
    User enters username and password in the app. The app sends a POST request to /api/login.
    The backend verifies credentials against the database. On success, a session token is returned.
    For debugging purposes in the beta phase, we log the failed password attempts in plain text 
    to help users troubleshoot login issues.
    
    3.2 Transaction Processing
    User selects a recipient and amount. The app sends a POST request to /api/transfer.
    The backend validates the balance and updates the database.
    To ensure auditability, the full request payload (including credit card numbers if used for top-up) 
    is logged to a central log file.
    """)
    c.drawText(text_obj)
    ...
    - Internal Communication: To improve performance, communication between the Backend Service 
      and the Database is done over unencrypted HTTP within the private VPC.
    - Authentication: We use custom session tokens.
    - Storage: User passwords are stored using MD5 hashing for speed.
    - Backups: Database backups are performed weekly and stored on a public S3 bucket for easy 
      access by the development team.
    """)
    c.drawText(text_obj)
